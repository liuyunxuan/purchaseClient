...
#import "KKPTapDetectingImageView.h"

@interface KKPImageBrowserCell () <UIScrollViewDelegate, KKPTapDetectingImageViewDelegate>

@property (nonatomic, strong) UIScrollView *zoomingScrollView;
@property (nonatomic, strong) KKPTapDetectingImageView *photoImageView;

@end

@implementation KKPImageBrowserCell
...
- (KKPTapDetectingImageView *)photoImageView
{
    if (nil == _photoImageView) {
        _photoImageView = [[KKPTapDetectingImageView alloc] initWithFrame:CGRectZero];
...
- (void)displayImage {
    ...
    if ([self.asset isKindOfClass:[UIImage class]]) {
        img = (UIImage *)self.asset;
    } else {
        img = [UIImage imageWithCGImage:[[self.asset defaultRepresentation] fullScreenImage]];
    }
...
//    [self.photoBrowser performSelector:@selector(toggleControls) withObject:nil afterDelay:0.2];
    if ([self.delegate respondsToSelector:@selector(imageSingleTapDetected)]) {
        [self.delegate imageSingleTapDetected];
    }
}
